...
public class TransmutationData {
    ...
    public void save(ValueOutput compound) {
        ValueOutput.ValueOutputList listOut = compound.childrenList("TransmutationData");
        for (Object2DoubleMap.Entry<ItemStack> entry : itemstackData.object2DoubleEntrySet()) {
            ValueOutput tag = listOut.addChild();
            tag.store("Item", ItemStack.CODEC, entry.getKey());
            tag.putDouble("Weight", entry.getDoubleValue());
        }
    }

    public static TransmutationData load(ValueInput compound) {
        TransmutationData data = new TransmutationData();
        for (ValueInput innerTag : compound.childrenListOrEmpty("TransmutationData")) {
            try {
                innerTag.read("Item", ItemStack.CODEC).ifPresent(from -> {
                    if (!from.isEmpty()) {
                        data.putWeight(from, innerTag.getDoubleOr("Weight", 0.0));
                    }
                });
            } catch (Exception e) {
                e.printStackTrace();
