...
public class RenderPotoo extends MobRenderer<EntityPotoo, LivingEntityRenderState, CitadelEntityModelBridge<EntityPotoo>> {
    ...
    public RenderPotoo(EntityRendererProvider.Context renderManagerIn) {
        super(renderManagerIn, new CitadelEntityModelBridge<>(new ModelPotoo()),  0.35F);
    }
    ...
    protected void scale(LivingEntityRenderState state, PoseStack matrixStackIn) {
        EntityPotoo eagle = AlexsMobsClientKeys.getLiving(state) instanceof EntityPotoo p ? p : null;
        if (eagle == null) {
            return;
        }
