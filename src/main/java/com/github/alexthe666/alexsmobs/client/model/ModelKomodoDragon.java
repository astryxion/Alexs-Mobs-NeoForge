...
public class ModelKomodoDragon extends AlexAdvancedEntityModel<EntityKomodoDragon> {
	...
	/** Copies live pose from another komodo model (same mesh layout), used by {@link com.github.alexthe666.alexsmobs.client.render.RenderKomodoDragon.LayerSaddle}. */
	public void copyPropertiesTo(ModelKomodoDragon target) {
		java.util.Iterator<AdvancedModelBox> src = this.getAllParts().iterator();
		java.util.Iterator<AdvancedModelBox> dst = target.getAllParts().iterator();
		while (src.hasNext() && dst.hasNext()) {
			AdvancedModelBox from = src.next();
			AdvancedModelBox to = dst.next();
			...
			to.scaleChildren = from.scaleChildren;
		}
	}
