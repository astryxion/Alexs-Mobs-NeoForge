...
public class ModelKangaroo extends AlexAdvancedEntityModel<EntityKangaroo> {
	...
	/**
	 * Copies animated pose from this model to a vanilla humanoid armor model used by {@code LayerKangarooArmor}.
	 */
	public void copyArmorPoseToHumanoid(net.minecraft.client.model.HumanoidModel<?> dest) {
		copyPart(dest.head, this.head);
		copyPart(dest.hat, this.head);
		copyPart(dest.body, this.body);
		copyPart(dest.leftArm, this.arm_left);
		copyPart(dest.rightArm, this.arm_right);
		copyPart(dest.leftLeg, this.leg_left);
		copyPart(dest.rightLeg, this.leg_right);
	}

	private static void copyPart(ModelPart dest, AdvancedModelBox src) {
		dest.xRot = src.rotateAngleX;
		dest.yRot = src.rotateAngleY;
		dest.zRot = src.rotateAngleZ;
	}
}