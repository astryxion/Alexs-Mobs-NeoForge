...
public class RenderCentipedeBody extends MobRenderer<EntityCentipedeBody, LivingEntityRenderState, CitadelEntityModelBridge<EntityCentipedeBody>> {
    ...
    public RenderCentipedeBody(EntityRendererProvider.Context renderManagerIn) {
        super(renderManagerIn, new CitadelEntityModelBridge<>(new ModelCaveCentipede<>(1)), 0.5F);
    }
    ...
    protected void setupRotations(LivingEntityRenderState state, PoseStack stack, float ageInTicks, float rotationYaw) {
        EntityCentipedeBody entity = AlexsMobsClientKeys.getLiving(state) instanceof EntityCentipedeBody b ? b : null;
        if (entity == null) {
