...
public class EntityMimicube extends Monster implements RangedAttackMob {
    ...
    public void performRangedAttack(LivingEntity target, float distanceFactor) {
        ...
        if (this.getMainHandItem().getItem() instanceof net.minecraft.world.item.BowItem bowItem) {
            abstractarrowentity = bowItem.customArrow(abstractarrowentity, ItemStack.EMPTY, ItemStack.EMPTY);
        }
        double d0 = target.getX() - this.getX();
    ...
    public boolean isBlocking() {
        ItemAbility shieldBlock = ItemAbility.get("shield_block");
        return this.getMainHandItem().canPerformAction(shieldBlock) || this.getOffhandItem().canPerformAction(shieldBlock);
    }
    ...
    public void tick() {
        ...
        if (this.onGround() && !this.wasOnGround) {
            ...
            for (int j = 0; j < 8; ++j) {
                ...
                this.level().addParticle(new ItemParticleOption(ParticleTypes.ITEM, AMItemRegistry.MIMICREAM.get()), this.getX() + (double)f2, this.getY(), this.getZ() + (double)f3, 0.0D, 0.0D, 0.0D);
            }
        ...
        if (this.entityData.get(ATTACK_TICK) > 0) {
            if (this.entityData.get(ATTACK_TICK) == 2 && this.getTarget() != null && this.distanceTo(this.getTarget()) < 2.3D && this.level() instanceof ServerLevel serverLevel) {
