...
public class EntityJerboa extends Animal {
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (type != InteractionResult.SUCCESS && !isFood(itemstack) && itemstack.is(AMTagRegistry.JERBOA_BEGS_FOR)) {
            ...
            if (random.nextFloat() <= 0.3F) {
                player.addEffect(new MobEffectInstance(AMEffectRegistry.FLEET_FOOTED.getDelegate(), 12000));
            }
            return InteractionResult.SUCCESS;
        }
        return type;
    ...
    public boolean hurtServer(net.minecraft.server.level.ServerLevel level, DamageSource source, float amount) {
        ...
        if (prev) {
            this.setSleeping(false);
            if (source.getEntity() instanceof LivingEntity hurter && hurter.hasEffect(AMEffectRegistry.FLEET_FOOTED)) {
                hurter.removeEffect(AMEffectRegistry.FLEET_FOOTED.getDelegate());
            }
        }
        return prev;
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel p_146743_, AgeableMob p_146744_) {
        EntityJerboa boa = AMEntityRegistry.JERBOA.get().create(p_146743_, EntitySpawnReason.BREEDING);
