...
public class EntityAlligatorSnappingTurtle extends Animal implements ISemiAquatic, Shearable, net.neoforged.neoforge.common.IShearable {
    ...
    public void shear(ServerLevel level, SoundSource category, ItemStack stack) {
        ...
        if (!this.level().isClientSide()) {
            if (random.nextFloat() < this.getMoss() * 0.05F) {
                this.spawnAtLocation(level, AMItemRegistry.SPIKED_SCUTE.get());
            } else {
    ...
    public AgeableMob getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
        return AMEntityRegistry.ALLIGATOR_SNAPPING_TURTLE.get().create(p_241840_1_, EntitySpawnReason.BREEDING);
    }
}
