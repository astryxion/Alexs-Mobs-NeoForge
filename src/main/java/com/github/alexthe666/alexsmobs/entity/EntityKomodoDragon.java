...
public class EntityKomodoDragon extends TamableAnimal implements ITargetsDroppedItems, IFollower {
    ...
    private static final EntityDataAccessor<Optional<UUID>> JOSTLER_UUID = SynchedEntityData.defineId(EntityKomodoDragon.class, AMEntityRegistry.OPTIONAL_UUID_SERIALIZER.get());
    ...
    public void readAdditionalSaveData(ValueInput compound) {
        ...
        if (((ValueInputExtension) (Object) compound).keySet().contains("SpitTime")) {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide() && this.isAlive() && !this.isBaby() && --this.timeUntilSpit <= 0) {
            if (this.level() instanceof ServerLevel serverLevel) {
                this.spawnAtLocation(serverLevel, AMItemRegistry.KOMODO_SPIT.get());
            }
    ...
    public AgeableMob getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
        return AMEntityRegistry.KOMODO_DRAGON.get().create(p_241840_1_, EntitySpawnReason.BREEDING);
    }
