...
public class MantisShrimpAIBreakBlocks extends Goal {
    ...
    private void breakBlock() {
        if (shouldMoveTo(mantisShrimp.level(), destinationBlock)) {
            ...
            if (mantisShrimp.level() instanceof net.minecraft.server.level.ServerLevel serverLevel && !mantisShrimp.level().isEmptyBlock(destinationBlock) && net.neoforged.neoforge.common.CommonHooks.canEntityDestroy(serverLevel, destinationBlock, mantisShrimp) && state.getDestroySpeed(mantisShrimp.level(), destinationBlock) >= 0) {
