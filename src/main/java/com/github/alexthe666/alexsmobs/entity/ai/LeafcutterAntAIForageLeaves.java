...
public class LeafcutterAntAIForageLeaves extends MoveToBlockGoal {
    ...
    private void breakLeaves() {
        ...
        if (blockstate.is(AMTagRegistry.LEAFCUTTER_ANT_BREAKABLES)) {
            if (this.ant.level() instanceof net.minecraft.server.level.ServerLevel serverLevel && net.neoforged.neoforge.event.EventHooks.canEntityGrief(serverLevel, this.ant)) {
            ...
            for (int l = 0; l < i; ++l) {
                for (int i1 = 0; i1 <= l; i1 = i1 > 0 ? -i1 : 1 - i1) {
                    for (int j1 = i1 < l && i1 > -l ? l : 0; j1 <= l; j1 = j1 > 0 ? -j1 : 1 - j1) {
                        ...
                        if (this.ant.isWithinHome(blockpos$mutableblockpos) && this.isValidTarget(this.mob.level(), blockpos$mutableblockpos)) {
