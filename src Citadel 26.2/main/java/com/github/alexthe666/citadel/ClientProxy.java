...
public class ClientProxy extends ServerProxy {
    ...
    public void playerRender(RenderPlayerEvent.Pre event) {
        ...
        if (!player.isModelPartShown(PlayerModelPart.CAPE) || event.isCanceled() || player.isSpectator()) {
            return;
        }
        if (Citadel.PATREONS.contains(username)) {
    ...
    @SubscribeEvent
    public void renderWorldLastOpaque(RenderLevelStageEvent.AfterOpaqueBlocks event) {
        if (Pathfinding.isDebug()) {
            WorldEventContext.INSTANCE.renderWorldLastAfterOpaque(event);
        }
    }

    @SubscribeEvent
    public void renderWorldLastTranslucent(RenderLevelStageEvent.AfterTranslucentBlocks event) {
        if (Pathfinding.isDebug()) {
            WorldEventContext.INSTANCE.renderWorldLastAfterTranslucent(event);
        }
    }
    ...
    @SubscribeEvent
    public void citadelPostEffectAfterLevel(RenderLevelStageEvent.AfterLevel event) {
